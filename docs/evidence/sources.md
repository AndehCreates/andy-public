# Sanitized source identity record

This record intentionally omits local checkout locations. Availability, observed
identity, commit, and inspection date are completed from the Task 5 inventories.

| Project ID | Availability | Source availability | Observed public remote | Commit | Inspection date |
| --- | --- | --- | --- | --- | --- |
| project-chief-of-staff | available | local-only | none authorized | 884ecb0 | 2026-07-22 |
| project-lifeos | available | public | https://github.com/AndehCreates/LifeOS | 7f50ad1 | 2026-07-21 |
| project-mathpad | available | public | https://github.com/AndehCreates/mathpad | 6704730 | 2026-07-21 |
| project-arcade | available | public | https://github.com/AndehCreates/arcade | 0490ff0 | 2026-07-21 |
| project-alpha-screener | available | public | https://github.com/AndehCreates/alpha-screener | 787aaef | 2026-07-21 |
| project-adhd-tabs | available | local-only | none observed | 60b897b | 2026-07-21 |
| project-creative-suite | available | local-only | none observed | 53824f1 | 2026-07-21 |
| project-android-lab | available | local-only | none observed | 1f3186b | 2026-07-21 |
| project-japanese-language-cognition | available | public | https://github.com/AndehCreates/japanese-anime-inspired | 6ebf6e8 | 2026-07-22 |
| project-second-brain | available | local-only | none authorized | fa5f8b9 | 2026-07-22 |
| project-nexus | source-unconfirmed | unconfirmed | none authorized | not observed | 2026-07-22 |
| project-ai-hub | source-unconfirmed | unconfirmed | none authorized | not observed | 2026-07-22 |
