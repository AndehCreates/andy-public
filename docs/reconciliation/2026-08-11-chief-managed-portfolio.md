...
| Creative Suite | Public `main` at `a2b255c17c5aa437e65e18b0cbd68c2fc5f55f89`; existing inventory is local-only `53824f1` | No project entry; old draft review row | Missing required evidence | Inspect the current public source, replace the local-only inventory with a current sanitized inventory, add a pending review row, and wait for explicit approval before a listed entry. |
